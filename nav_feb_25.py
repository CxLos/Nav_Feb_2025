...
# Check and convert any remaining Timestamp objects to strings
# for col in df.select_dtypes(include=['datetime64[ns]', 'datetime']):
#     df[col] = df[col].dt.strftime('%Y-%m-%d')

# for col in df1.select_dtypes(include=['datetime64[ns]', 'datetime']):
#     df1[col] = df1[col].dt.strftime('%Y-%m-%d')

# for col in df_q1.select_dtypes(include=['datetime64[ns]', 'datetime']):
#     df_q1[col] = df_q1[col].dt.strftime('%Y-%m-%d')

# # Format as string
# df['Date of activity:'] = df['Date of activity:'].dt.strftime('%Y-%m-%d')
# df_q1['created_at'] = df_q1['created_at'].dt.strftime('%Y-%m-%d')
# df_q1.loc[:, 'created_at'] = df_q1['created_at'].dt.strftime('%Y-%m-%d')
...
# df_q1_json = df_q1.to_dict(orient='records')

# # Serialize to JSON
# df_json_str = json.dumps(df_json, indent=4)
# df1_json_str = json.dumps(df1_json, indent=4)
# df_q1_json_str = json.dumps(df_q1_json, indent=4)

# # Print JSON strings
# print(df_json_str)
# print(df1_json_str)
# print(df_q1_json_str)
...
# # Cleaning up the 'Person submitting this form:' column
df['Person submitting this form:'] = df['Person submitting this form:'].str.strip()
df['Person submitting this form:'] = df['Person submitting this form:'].replace('Dominique', 'Dominique Street')
df['Person submitting this form:'] = df['Person submitting this form:'].replace('Jaqueline Ovieod', 'Jaqueline Oviedo')
df['Person submitting this form:'] = df['Person submitting this form:'].replace('Sonya', 'Sonya Hosey')
...
df['ZIP Code:'] = df['ZIP Code:'].replace("Texas", mode_value)
        ...
        html.Div(
            ...
            html.Div(
                ...
                children=[
                    html.A(
                        'Repo',
                        href='https://github.com/CxLos/Nav_Feb_2025',
                        className='btn'),
                ]),
    ]),  
